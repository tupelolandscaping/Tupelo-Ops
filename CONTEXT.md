...
- **Relay** — the business banking platform. Bank statements (actual cash movement) are the truest cash signal and the third leg of reconciliation (model ↔ CRM ↔ bank). Five-account Profit First structure — 8735 Income, 3549 OpEx, 3550 Reserve, 8737 Taxes; 8736 (Owner's Comp) deleted in Relay 2026-07-05. Only Income and OpEx are historically active; the other three confirmed $0 across all 17 months of statements (canonical account map: reference/README.md — update there first if accounts change). Consolidation is an early post-transition workstream (Section 7 #9) that **gates the first model build**.
...
- **E. Rebuild the financial model** (sequenced in Section 10, after items B–D, F, H, I, J and after the remaining pre-build gate — bank consolidation, Follow-Up #9) per the architecture in Section 9: atomic ledger with separate billing/payment events (D5a); itemized labor (D2); materials/fuel placeholder (D1); two-phase crew-lead cost (D4); reconciliation tab with honestly-scoped coverage (D8). *Revenue export-format + service mapping (Follow-Up #6) was substantially resolved 2026-07-06 — the flat ledger exists and is reconciled — and no longer gates the build; the residual bundled-line classification gap (56.6% of gross) does not block it (see Follow-Up #6, `HISTORY.md` H-033, H-036).*
...
9. **Bank-statement (Relay) consolidation** — *substantially resolved (2026-07-05).* Account roles confirmed by owner (8735 Income, 3549 OpEx, 3550 Reserve, 8737 Taxes; 8736 Owner's Comp deleted); the three non-active accounts verified $0 across all 17 months, so consolidation is a clean two-account (Income + OpEx) carry-forward. **Still GATES the first model build** — the build waits for bank-verified cash rather than proceeding on CRM data alone — this is a hard gate, not a partial-path gate like #6, because #6 affects only how revenue is *classified* while #9 determines the *accuracy of cash itself*, which is bank-sourced (see D5a for the full reasoning). (Not to be started before the GitHub transition.)
...
16. **Catalog/practice tax inconsistency** — the catalog prices Subcontractor Work at 0% tax, but historical "Subcontractor Planning" lines were charged 6%; conversely Debris Removal/Bagging is a 6% catalog item, yet two "Bagging" lines were charged 0%. The surcharge was applied inconsistently against the catalog's own rates. Dollar totals are unaffected (the surcharge is stored per-line as actually charged, not recomputed). Relevant to #10's surcharge-structure decision. Do not correct historical records.
...
5. **Clear the remaining pre-build gate** — **bank-statement consolidation** (Follow-Up #9): consolidate the Relay accounts and establish the full cash picture. (Gate B — revenue export-format + service mapping, Follow-Up #6 — cleared 2026-07-06: the flat transaction-line export exists and is reconciled; see `HISTORY.md` H-033. The residual bundled-line classification gap is tagged `BLOCKED — unmapped` per Section 9 but does not block the build.)
