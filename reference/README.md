...
**Refresh cadence — flagged, not decided; now feeds the pipeline (as of H-062).** `model/match_payments.py`'s `load_stripe_fee_lookup()` now reads every `stripe-balance-history-*.csv` file present and uses it as the ground-truth source for Stripe fee amounts on any payout it covers (true per-charge sums, replacing the 3.9%+$0.30 back-solved estimate for those rows), falling back to the formula only for payouts this export doesn't cover. This means the file is no longer a one-time reconciliation aid only — it is now a live pipeline input, and the coverage gap (dates after 2026-07-03) will start silently reverting to the less-accurate fallback the moment new Stripe payouts occur past that date. **Whether this file needs its own dated-snapshot + active-pointer + `*-UPDATE.md` refresh procedure (identical to the payroll and Relay exports, per `HISTORY.md` H-048/H-039) is still not decided** — but it is now materially more load-bearing than when this section was first written, since the pipeline's fee accuracy for future months depends on someone periodically re-pulling this export the same way Relay and payroll already are. Flagged as a live gap: absent a refresh, the pipeline's own fallback keeps working (never fails), but silently degrades in precision for any payout after 2026-07-03.
