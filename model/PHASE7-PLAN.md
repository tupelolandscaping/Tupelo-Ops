...
**Update, 2026-07-12 (H-061/H-064):** "from Relay data alone" turned out to be the operative qualifier — a *separate* source (Stripe's own transaction export, `reference/stripe-balance-history-*.csv`) carries exactly the invoice-citing detail Relay's own export lacks, and its `Description` field cites invoice numbers directly, structurally the same signal Finding 1 already uses for tier-1 matching. Recomputed against the full ledger (a wider, more precise base than this Finding's original Jan–Jul CRM cross-check): the Stripe-aggregated share is **32.83%** ($9,856.20 of $30,023.25), and all of it — 32/32 payouts — is now demonstrated re-attributable using that export (`CONTEXT.md` Follow-Up #25). Not yet built; Finding 3's tier-3 design below remains what's actually implemented today.
...
**Update, 2026-07-12 (H-062, resolving Follow-Up #17 per H-063):** the back-solved `gross = (net + $0.30) / (1 - 0.039)` formula above was always a stand-in for missing per-charge visibility, not a deliberate design choice on its own merits — confirmed by the retrospective this update records. Once `reference/stripe-balance-history-*.csv` (H-061) made real per-charge fees available, `model/match_payments.py` was corrected to sum the *true* per-charge fees for any payout that export covers, keeping the back-solved formula only as an explicit fallback for payouts it doesn't (H-062) — this is what Sequencing Step 3 would have built from day one had this export existed then; no other part of this paragraph's design (payment event `amount` = actual net deposit, never a back-computed gross) would have differed. Follow-Up #17 itself is now fully resolved (H-063): the 3.9%+$0.30 combined rate was always correct, and is now known to compose as 2.9%+$0.30 true Stripe cost + a 1% Homeworks platform markup (all plans below Enterprise) — no dollar figure in this ledger changes as a result.
...
**Labor.** *Superseded, 2026-07-10 (H-049 — see CONTEXT.md D2's reframing and Follow-Ups #19–20).* Labor is now real, ACTUAL, per-employee, per-pay-period data sourced directly from the two Gusto payroll journal exports in `reference/` (`model/data/employee-role-map.csv` for time-varying role attribution; `model/populate_labor_from_payroll.py` for the loader; `model/data/ledger-labor.csv` for the resulting rows). The $25/$20, 2-person 50/50-split model is now a forward-looking projection-layer assumption only, not the source of historical labor cost. The earlier "aggregate-only sanity check, cannot override the estimate" framing is obsolete: a **full-history reconciliation gate** now checks every non-reversed, non-empty pay period's Net Pay and combined Employee+Employer Tax against a specific matching Relay `GUSTO` `NET`/`TAX` transaction — **24 of 25 periods matched exactly on both figures**; the sole exception is a $5.37 employer-tax-only correction (payday 2026-03-31, a VA Unemployment true-up with zero wages) that could not be traced to any discrete Relay transaction (Follow-Up #20), booked as ACTUAL from the payroll source regardless since the figure itself is not in doubt. One further known scope caveat carries forward from D2/Follow-Up #19: payroll actuals have no per-job/per-customer attribution, and they cover all hours combined (recurring + one-off project work), broader than the original recurring-only estimate.
