...
- [ ] Define the **data-layer CSVs** in `model/data/`: the atomic ledger (schema `date | type | event | category | subcategory | customer | quantity | unit_rate | amount | status | source`, with **separate invoice and payment events** per D5a) and the assumption tables. *(Schema created but unpopulated — H-041 (2026-07-09) added five schema-only skeleton files, `model/data/ledger-revenue.csv`, `ledger-labor.csv`, `ledger-overhead.csv`, `ledger-materials.csv`, `ledger-capital.csv`, each carrying only the header row (`date,type,event,category,subcategory,customer,quantity,unit_rate,amount,status,source`) with no data rows. `model/data/` also still holds the revenue-ledger CSVs and the two catalog-mapping files built for Gates A/B. Left unchecked because the ledger is not yet populated — the schema step itself has started.)*
