...
def build_plan_vs_actual(ws, ledgers, months, assumptions):
    ...
    ws.append(["  - Anais's regular wage ($25/hr, real in payroll/ledger data) is expected to drop to"])
    ws.append(["    $20/hr, but this has NOT happened yet -- no effective date exists (pending only,"])
    ws.append(["    same treatment as Konji's not-yet-run rate change). No row or ledger change was"])
    ws.append(["    made for this; untouched until the owner confirms it has occurred."])
