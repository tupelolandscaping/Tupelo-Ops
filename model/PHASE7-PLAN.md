...
**Proposed (not yet built) — a `type=overhead, subcategory=payment-processing-fee` ledger category.** Per Finding 7, the Stripe fee (3.9% + $0.30/charge) is a real, computable cost distinct from both the surcharge and any Follow-Up #15 gap. Once tier 3 above is matching real Stripe deposits, each can generate a paired fee event: the gross card charge (back-computed from the net deposit and the known formula) as the true revenue-side amount, and the fee itself as a `type=overhead` cost event — keeping the three phenomena in Finding 7 structurally separate rather than netted into one ambiguous "shortfall" number. **This portion of the design is partially BLOCKED**: the Stripe-fee side can proceed once payment-event matching begins (the formula is known), but fee-netting cannot be completed for Homeworks-direct card payments until the Homeworks fee schedule is supplied (Follow-Up #17) — do not invent or estimate that figure in the interim.
...
6. **Payment-processing-fee ledger category (Section 3b, Finding 7).** Not a choice awaiting a preference — a genuine external-data BLOCKED item. The Stripe-fee portion (3.9% + $0.30/charge, owner-confirmed) can be built once payment-event matching begins; the Homeworks-direct-card-payment portion is **BLOCKED** until a fee schedule is supplied (new Follow-Up #17 in `CONTEXT.md`). Do not estimate or invent this figure to unblock it.
