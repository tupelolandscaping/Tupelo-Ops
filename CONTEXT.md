...
6. **Revenue-by-service mapping** — *substantially resolved (2026-07-06).* The flat transaction-line ledger exists (`model/data/revenue-line-items.csv`, 152 rows extracted from the 68 source invoices), reconciled to the Homeworks billed anchor to the penny. The legacy→catalog name mapping is now resolved (`model/data/service-name-map.csv`, 13 owner-confirmed renames; every line item carries both the literal `service_raw` header and the mapped `service`) — revenue-by-service is now a meaningful derived view. The only residual gap: 56.6% of revenue ($15,795.85 of $27,891.65) sits in bundled service lines ("General Maintenance," "Lawn Care," "Lawn Maintenance") that cannot be decomposed into catalog services at the line level — a limitation of historical invoicing practice, not of the data (see Follow-Up #13 for the forward fix). Service-resolved: 43.4%. **This gate no longer blocks the first model build** (see `HISTORY.md` H-030, H-033, H-036, H-037).
...
13. **Line-item billing going forward** — owner action: adjust all recurring visits to be billed by individual line item rather than bundled ("General Maintenance"), so future revenue is service-resolved at the source. This is the forward fix for the bundling ceiling identified in #6; it does not recover historical bundles.
