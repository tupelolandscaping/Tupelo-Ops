...
6. **Revenue-by-service mapping** — *substantially resolved (2026-07-06).* The flat transaction-line ledger exists (`reference/revenue-line-items.csv`, 152 rows extracted from the 68 source invoices), reconciled to the Homeworks billed anchor to the penny. Remaining open: 40.7% of revenue ($11,339.40 of $27,891.65) sits in bundled service lines ("General Maintenance," "Lawn Care," "Lawn Maintenance") that cannot be decomposed into catalog services at the line level — a limitation of historical invoicing practice, not of the data (see Follow-Up #13 for the forward fix). Service-resolved: 59.3%. **This gate no longer blocks the first model build** (see `HISTORY.md` H-030, H-033).
...
15. **Billed-vs-collected tension** — every one of the 68 invoices in `reference/revenue-invoices.csv` shows Amount Paid = This Invoice and Total Due = 0.00, yet the CRM reports $12,726.01 billed vs. $10,973.57 collected for Jan–Jul 2026. Likely a CRM timing/recording artifact, but unverified — do not assert a cause. Resolve during the model build against the bank statements.
...
- **Revenue** — traceable to CRM revenue exports as invoice/payment events. The flat transaction-line ledger now exists (`reference/revenue-line-items.csv`, 152 rows, reconciled to the Homeworks billed anchor to the penny), so revenue can be fully populated from it — this no longer gates the build (Follow-Up #6, resolved 2026-07-06; see Section 10 step 5). The `BLOCKED — unmapped` tag is narrowed accordingly: it applies only to service classification on bundled lines ("General Maintenance," "Lawn Care," "Lawn Maintenance" — 40.7% of gross) that cannot be decomposed into catalog services, not to revenue amounts, dates, or customers, which are all ACTUAL regardless of bundling. Collected events are cross-checked against Relay bank statements once consolidated (Follow-Up #9).
